...
contains
  ...
  subroutine PrintHybridnew(hybt,vname)
    type (hybrid_t),  intent(in) :: hybt
    character(len=*), intent(in) :: vname

    write(iulog,21) vname, hybt%par%rank, hybt%ithr, hybt%nthreads, &
  ...
  function config_thread_region_par(par,region_name) result(hybrid)
      ...
      if (TRIM(region_name) == 'serial') then
         ...
         if (.not. allocated(work_pool_horz)) then
            allocate(work_pool_horz(horz_num_threads,2), stat=iret, errmsg=errorstring)
      ...
      if (TRIM(region_name) == 'horizontal') then
         ...
         call set_thread_ranges_1D (work_pool_horz, ibeg_range, iend_range, ithr)
         hybrid%ibeg = ibeg_range
         hybrid%iend = iend_range
         hybrid%kbeg = 1
         hybrid%kend = nlev
         hybrid%qbeg = 1
         hybrid%qend = qsize
      ...
      if (TRIM(region_name) == 'vertical') then
         ...
         hybrid%ibeg = 1
         hybrid%iend = nelemd_save
         hybrid%kbeg = kbeg_range
         hybrid%kend = kend_range
         hybrid%qbeg = 1
         hybrid%qend = qsize
      end if
      ...
      if (TRIM(region_name) == 'tracer') then
         ...
         hybrid%kbeg = 1
         hybrid%kend = nlev
         hybrid%qbeg = qbeg_range
         hybrid%qend = qend_range
      end if
...
!      hybrid%par          = par      ! relies on parallel_mod copy constructor
  ...
  subroutine init_loop_ranges(nelemd)
      ...
      if (init_ranges) then
        ...
        if(nelemd<horz_num_threads) then
          write(iulog,*) 'WARNING: insufficient horizontal parallelism to support ',horz_num_threads,' horizontal threads'
        end if
        ...
        if(nlev<vert_num_threads) then
          write(iulog,*) 'WARNING: insufficient vertical parallelism to support ',vert_num_threads,' vertical threads'
        end if
        if (.not. allocated(work_pool_vert)) then
          allocate(work_pool_vert(vert_num_threads,2), stat=iret, errmsg=errorstring)
        ...
        if(qsize<tracer_num_threads) then
          write(iulog,*) 'WARNING: insufficient tracer parallelism to support ',tracer_num_threads,' tracer threads'
        end if
        if (.not. allocated(work_pool_trac)) then
          allocate(work_pool_trac(tracer_num_threads,2), stat=iret, errmsg=errorstring)
        ...
        if(use_cslam .and. ntrac<tracer_num_threads) then
          write(iulog,*) 'WARNING: insufficient CSLAM tracer parallelism to support ',tracer_num_threads,' tracer threads'
        end if
        if (.not. allocated(work_pool_ctrac)) then
          allocate(work_pool_ctrac(tracer_num_threads,2), stat=iret, errmsg=errorstring)
 ...
 subroutine set_region_num_threads(local_name)
  ...
  type (hybrid_p), intent(inout) :: pybrid
  ...
  if (TRIM(region_name) == 'horizontal') then
    ...
    pybrid%ibeg = ibeg_range
    pybrid%iend = iend_range
    pybrid%kbeg = 1
    pybrid%kend = nlev
    pybrid%qbeg = 1
    pybrid%qend = qsize
  ...
  if (TRIM(region_name) == 'vertical') then
    ...
    pybrid%kbeg = kbeg_range
    pybrid%kend = kend_range
    pybrid%qbeg = 1
    pybrid%qend = qsize
  end if
  ...
  if (TRIM(region_name) == 'tracer') then
    ...
    pybrid%kbeg = 1
    pybrid%kend = nlev
    pybrid%qbeg = qbeg_range
    pybrid%qend = qend_range
  end if
...
!  write(iulog,1000) idthread, beg_range_1, end_range_1, &
...
! write(iulog,1000) idthread, beg_range, end_range
! call flush(iulog)
