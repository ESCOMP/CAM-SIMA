module se_dyn_time_mod
  ...
  real (kind=r8)           , public :: tevolve       ! time evolved since start of dynamics (end of physics)
  real (kind=r8)           , public :: phys_tscale=0 ! Physics time scale
  real (kind=r8)           , public :: dt_phys = -900! physics time-step (only used in standalone HOMME)
                                                 ! if negative no forcing (see prim_main)
