...
!    elem%normDinv=max_normDinv/sqrt(2)
    ...
    elem%dx_short = 1.0_r8/(max_svd*0.5_r8*real(np-1, r8)*ra*1000.0_r8)
    elem%dx_long  = 1.0_r8/(min_svd*0.5_r8*real(np-1, r8)*ra*1000.0_r8)
  ...
  subroutine vmap(D, x1, x2, face_no)
    ...
    if (face_no >= 1 .and. face_no <= 4) then
       ...
       write(errmsg, '(a,i0)') 'VMAP: Bad face number, ',face_no
       call endrun(errmsg)
     end if
