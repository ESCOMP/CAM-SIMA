module cam_grid_support
   ...
   type, public :: horiz_coord_t
      ...
      type(var_desc_t), pointer :: bndsvdesc => NULL() ! Set to write bounds
   contains
   ...
   type, extends(cam_grid_attribute_t) :: cam_grid_attribute_1d_int_t
      ...
      integer(iMap),  pointer             :: map(:) => NULL() ! map (dof) 4 I/O
   contains
   ...
   type, public :: cam_grid_patch_t
      private
      integer                      :: grid_id = -1  ! grid with patch points
      ...
      type(cam_filemap_t), pointer :: mask       => null() ! map 4 active pts
      integer(iMap),       pointer :: latmap(:)  => null() ! map 4 patch coords
      integer(iMap),       pointer :: lonmap(:)  => null() ! map 4 patch coords
      real(r8),            pointer :: lonvals(:) => null() ! collected output
      real(r8),            pointer :: latvals(:) => null() ! collected output
   contains
   ...
   ! Access functions for grids rely on the grid's ID or name, not its index.
   ...
   end interface cam_grid_write_dist_array
   ...
   end interface get_cam_grid_index
...
contains
   ...
   subroutine write_horiz_coord_attr(this, File, dimid_out)
      ...
      integer                             :: bnds_dimid ! PIO dim for bounds
      ...
      if (ierr /= PIO_NOERR) then
         ...
         ! long_name
         ierr=pio_put_att(File, this%vardesc, 'long_name',                    &
              trim(this%long_name))
         call cam_pio_handle_error(ierr,                                      &
         ...
         if (associated(this%bnds)) then
            ...
            ierr = pio_put_att(File, this%vardesc, 'bounds',                  &
            ...
            ! long_name
            ierr = pio_put_att(File, this%bndsvdesc, 'long_name',             &
                 trim(this%name)//' bounds')
            call cam_pio_handle_error(ierr,                                   &
                 'Error writing bounds "long_name" attr in write_horiz_coord_attr')
            ! fill value
            ierr = pio_put_att(File, this%vardesc, '_FillValue',              &
                 grid_fill_value)
            call cam_pio_handle_error(ierr,                                   &
                 'Error writing "_FillValue" attr in write_horiz_coord_attr')
            ! units
            ierr = pio_put_att(File, this%bndsvdesc, 'units',                 &
                 trim(this%units))
            call cam_pio_handle_error(ierr,                                   &
   ...
   subroutine write_horiz_coord_var(this, File)
      use cam_pio_utils, only: cam_pio_get_decomp, cam_pio_newdecomp
      ...
      if (associated(this%vardesc)) then
         ...
         if (associated(this%map)) then
            ...
            if (associated(this%bnds) .and. associated(this%bndsvdesc)) then
               call cam_pio_newdecomp(iodesc, (/2, this%dimsize/),            &
                    this%map, pio_double)
               call pio_write_darray(File, this%bndsvdesc, iodesc,            &
                    this%bnds, ierr)
               call pio_syncfile(File)
               call pio_freedecomp(File, iodesc)
            end if
         ...
         write(errormsg, *) 'Error writing variable values for ',             &
              trim(this%name), ' in write_horiz_coord_var'
   ...
   subroutine cam_grid_register(name, id, lat_coord, lon_coord, map,          &
      ...
      if (get_cam_grid_index(trim(name)) > 0) then
         ...
         write(errormsg, '(4a,i5,3a)') trim(subname),                         &
              ': Attempt to add grid, ', trim(name), ' with id = ', id,       &
              ', however, grid ', trim(cam_grids(i)%name),                    &
              ' already has that ID'
   ...
   ! Given some array information, find the dimension NetCDF IDs on
   !    <File> for this grid
   ...
   subroutine cam_grid_get_decomp(id, field_lens, file_lens, dtype,           &
        iodesc, field_dnames, file_dnames)
      use pio,           only: io_desc_t

      ! Dummy arguments
      integer,                    intent(in)    :: id
      ! field_lens: Array dim sizes
      integer,                    intent(in)    :: field_lens(:)
      ! file_lens:  File dim sizes
      ...
      gridid = get_cam_grid_index(id)
      if (gridid > 0) then
         call cam_grids(gridid)%get_decomp(field_lens, file_lens, dtype,      &
              iodesc, field_dnames, file_dnames)
      else
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_read_dist_array_2d_int(File, id, adims, fdims,         &
        hbuf, varid)
      use pio, only: file_desc_t
      ...
      gridid = get_cam_grid_index(id)
      if (gridid > 0) then
         call cam_grids(gridid)%read_darray_2d_int(File, adims, fdims,        &
              hbuf, varid)
      else
         write(errormsg, *)                                                   &
              'cam_grid_read_dist_array_2d_int: Bad grid ID, ', id
         call endrun(errormsg)
      end if
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_read_dist_array_3d_int(File, id, adims, fdims,         &
        hbuf, varid)
      use pio, only: file_desc_t
      ...
      gridid = get_cam_grid_index(id)
      if (gridid > 0) then
         call cam_grids(gridid)%read_darray_3d_int(File, adims, fdims,        &
              hbuf, varid)
      else
         write(errormsg, *)                                                   &
              'cam_grid_read_dist_array_3d_int: Bad grid ID, ', id
         call endrun(errormsg)
      end if
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_read_dist_array_2d_double(File, id, adims, fdims,      &
        hbuf, varid)
      use pio, only: file_desc_t
      ...
      gridid = get_cam_grid_index(id)
      if (gridid > 0) then
         call cam_grids(gridid)%read_darray_2d_double(File, adims, fdims,     &
              hbuf, varid)
      else
         write(errormsg, *)                                                   &
              'cam_grid_read_dist_array_2d_double: Bad grid ID, ', id
         call endrun(errormsg)
      end if
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_read_dist_array_3d_double(File, id, adims, fdims,      &
        hbuf, varid)
      use pio, only: file_desc_t
      ...
      gridid = get_cam_grid_index(id)
      if (gridid > 0) then
         call cam_grids(gridid)%read_darray_3d_double(File, adims, fdims,     &
              hbuf, varid)
      else
         write(errormsg, *)                                                   &
              'cam_grid_read_dist_array_3d_double: Bad grid ID, ', id
         call endrun(errormsg)
      end if
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_read_dist_array_2d_real(File, id, adims, fdims,        &
        hbuf, varid)
      use pio, only: file_desc_t
      ...
      gridid = get_cam_grid_index(id)
      if (gridid > 0) then
         call cam_grids(gridid)%read_darray_2d_real(File, adims, fdims,       &
              hbuf, varid)
      else
         write(errormsg, *)                                                   &
              'cam_grid_read_dist_array_2d_real: Bad grid ID, ', id
         call endrun(errormsg)
      end if
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_read_dist_array_3d_real(File, id, adims, fdims,        &
        hbuf, varid)
      use pio, only: file_desc_t
      ...
      gridid = get_cam_grid_index(id)
      if (gridid > 0) then
         call cam_grids(gridid)%read_darray_3d_real(File, adims, fdims,       &
              hbuf, varid)
      else
         write(errormsg, *)                                                   &
              'cam_grid_read_dist_array_3d_real: Bad grid ID, ', id
         call endrun(errormsg)
      end if
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_write_dist_array_2d_int(File, id, adims, fdims,        &
        hbuf, varid)
      use pio, only: file_desc_t
      ...
      gridid = get_cam_grid_index(id)
      if (gridid > 0) then
         call cam_grids(gridid)%write_darray_2d_int(File, adims, fdims,       &
              hbuf, varid)
      else
         write(errormsg, *)                                                   &
              'cam_grid_write_dist_array_2d_int: Bad grid ID, ', id
         call endrun(errormsg)
      end if
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_write_dist_array_3d_int(File, id, adims, fdims,        &
        hbuf, varid)
      use pio, only: file_desc_t
      ...
      gridid = get_cam_grid_index(id)
      if (gridid > 0) then
         call cam_grids(gridid)%write_darray_3d_int(File, adims, fdims,       &
              hbuf, varid)
      else
         write(errormsg, *)                                                   &
              'cam_grid_write_dist_array_3d_int: Bad grid ID, ', id
         call endrun(errormsg)
      end if
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_write_dist_array_2d_double(File, id, adims, fdims,     &
        hbuf, varid)
      use pio, only: file_desc_t
      ...
      gridid = get_cam_grid_index(id)
      if (gridid > 0) then
         call cam_grids(gridid)%write_darray_2d_double(File, adims, fdims,    &
              hbuf, varid)
      else
         write(errormsg, *)                                                   &
              'cam_grid_write_dist_array_2d_double: Bad grid ID, ', id
         call endrun(errormsg)
      end if
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_write_dist_array_3d_double(File, id, adims, fdims,     &
        hbuf, varid)
      use pio, only: file_desc_t
      ...
      gridid = get_cam_grid_index(id)
      if (gridid > 0) then
         call cam_grids(gridid)%write_darray_3d_double(File, adims, fdims,    &
              hbuf, varid)
      else
         write(errormsg, *)                                                   &
              'cam_grid_write_dist_array_3d_double: Bad grid ID, ', id
         call endrun(errormsg)
      end if
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_write_dist_array_2d_real(File, id, adims, fdims,       &
        hbuf, varid)
      use pio, only: file_desc_t
      ...
      gridid = get_cam_grid_index(id)
      if (gridid > 0) then
         call cam_grids(gridid)%write_darray_2d_real(File, adims, fdims,      &
              hbuf, varid)
      else
         write(errormsg, *)                                                   &
              'cam_grid_write_dist_array_2d_real: Bad grid ID, ', id
         call endrun(errormsg)
      end if
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_write_dist_array_3d_real(File, id, adims, fdims,       &
        hbuf, varid)
      use pio, only: file_desc_t
      ...
      gridid = get_cam_grid_index(id)
      if (gridid > 0) then
         call cam_grids(gridid)%write_darray_3d_real(File, adims, fdims,      &
              hbuf, varid)
      else
         write(errormsg, *)                                                   &
              'cam_grid_write_dist_array_3d_real: Bad grid ID, ', id
         call endrun(errormsg)
      end if
   ...
   !  cam_grid_get_gcid
   !
   !  Find the global column ID for every local column
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_get_gcid(id, gcid)
      ...
      gridid = get_cam_grid_index(id)
      if ((gridid > 0) .and. (gridid <= cam_grid_num_grids())) then
   ...
   !------------------------------------------------------------------------
   !
   ! cam_grid_get_array_bounds: Return grid bounds for the relevant array
   ...
   subroutine cam_grid_compute_patch(id, patch, lonl, lonu, latl, latu, cco)
      ...
      logical,                         intent(in)    :: cco ! Collect cols?
   ...
   subroutine print_attr_base(this)
      ...
      class(cam_grid_attribute_t), intent(in)             :: this
      if (masterproc) then
         write(iulog, '(5a)') 'Attribute: ', trim(this%name),                 &
              ", long name = '", trim(this%long_name), "'"
      end if
   ...
   subroutine add_cam_grid_attribute_1d_int(gridname, name, long_name,        &
      ...
      if (gridind > 0) then
         ...
         if (associated(attptr)) then
            ...
            if (dimsize < 1) then
               write(errormsg, *)                                             &
                    'add_cam_grid_attribute_1d_int: attribute ',              &
                    'dimension ', trim(dimname), ' for ', trim(name),         &
                    ', not found'
               call endrun(errormsg)
            end if
            allocate(attr)
   ...
   subroutine add_cam_grid_attribute_1d_r8(gridname, name, long_name,         &
      ...
      if (gridind > 0) then
         ...
         if (associated(attptr)) then
            ...
            if (dimsize < 1) then
               write(errormsg, *)                                             &
                    'add_cam_grid_attribute_1d_r8: attribute ',               &
                    'dimension ', trim(dimname), ' for ', trim(name),         &
                    ', not found'
               call endrun(errormsg)
            end if
   ...
   subroutine initializeAttrPtr(this, attr)
      ...
      if (associated(this%next)) then
         if (masterproc) then
            write(iulog, *) 'WARNING: Overwriting attr pointer for ',         &
                 'cam_grid_attr_ptr_t'
         end if
      end if
      this%attr => attr
   ...
   subroutine write_cam_grid_attr_0d_int(attr, File)
      ...
      if (.not. associated(attr%vardesc)) then
         if (len_trim(attr%long_name) > 0) then
            ...
            ierr= pio_put_att(File, attr%vardesc, '_FillValue',               &
                 int(grid_fill_value))
            call cam_pio_handle_error(ierr,                                   &
                 'Error writing "_FillValue" attr in '//subname)
            ierr=pio_put_att(File, attr%vardesc, 'long_name',                 &
                 trim(attr%long_name))
            call cam_pio_handle_error(ierr,                                   &
            ...
            ierr = pio_inq_att(File, PIO_GLOBAL, attr%name, attrtype,         &
                 attrlen)
            if (ierr /= PIO_NOERR) then
               ! Time to define the attribute
               ierr = pio_put_att(File, PIO_GLOBAL, trim(attr%name),          &
                    attr%ival)
               call cam_pio_handle_error(ierr,                                &
                    'Unable to define attribute in '//subname)
            end if
         end if
      end if
   ...
   subroutine write_cam_grid_attr_0d_char(attr, File)
      ...
      integer(imap)                       :: attrlen
      integer                             :: ierr
      character(len=*), parameter :: subname = 'write_cam_grid_attr_0d_char'
   ...
   subroutine write_cam_grid_attr_1d_r8(attr, File)
      ...
      if (.not. associated(attr%vardesc)) then
         ...
         call cam_pio_def_var(File, trim(attr%name), pio_double,              &
              (/dimid/), attr%vardesc, existOK=.false.)
         ! fill value
         ierr =  pio_put_att(File, attr%vardesc, '_FillValue',                &
              grid_fill_value)
         call cam_pio_handle_error(ierr,                                      &
   ...
   subroutine cam_grid_attribute_copy(src_grid, dest_grid, attribute_name)
      ...
      if (associated(attr)) then
         ...
         write(errormsg, '(4a)') 'CAM_GRID_ATTRIBUTE_COPY: attribute ',       &
              trim(attribute_name), ' already exists for ',                   &
              cam_grids(dest_ind)%name
         call endrun(errormsg)
      else
   ...
   !  cam_grid_write_attr
   !
   !  Write the dimension and coordinate attributes for the horizontal
   !  history coordinates.
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_write_attr(File, grid_id, header_info)
      ...
      if (cam_grids(gridind)%attrs_defined) then
         ...
         ! Write the horizontal coord attributes first so that we have
         !   the dims
   ...
   subroutine write_cam_grid_val_0d_int(attr, File)
      ...
      if (associated(attr%vardesc)) then
         ...
         call cam_pio_handle_error(ierr, 'Error writing value in '//subname)
   ...
   subroutine write_cam_grid_val_1d_int(attr, File)
      use pio,           only: file_desc_t, pio_put_var, pio_int, io_desc_t
      use pio,           only: pio_inq_varid, pio_write_darray
      ...
      if (associated(attr%vardesc)) then
         ...
         if (associated(attr%map)) then
            ...
            call cam_pio_newdecomp(iodesc, (/attr%dimsize/), attr%map,        &
                 pio_int)
            call pio_write_darray(File, attr%vardesc, iodesc, attr%values,    &
                 ierr)
            call pio_freedecomp(File, iodesc)
            deallocate(iodesc)
            nullify(iodesc)
         ...
         call cam_pio_handle_error(ierr,                                      &
              'Error writing variable values in '//subname)
         deallocate(attr%vardesc)
         nullify(attr%vardesc)
      end if
   ...
   subroutine write_cam_grid_val_1d_r8(attr, File)
      ...
      use pio,           only: pio_inq_varid, pio_write_darray
      use pio,           only: io_desc_t, pio_freedecomp
      use cam_pio_utils, only: cam_pio_newdecomp
   ...
   !------------------------------------------------------------------------
   !
   !  cam_grid_dim_names: Return the names of the dimensions of the
   !                      grid axes.
   !        Note that these may be the same
   !
   !------------------------------------------------------------------------
   ...
   subroutine cam_grid_set_map(this, map, src, dest)
      ...
      call MPI_Allreduce(gridloc, gridlen, 1, MPI_INTEGER, MPI_SUM,           &
           mpicom, ierr)
   ...
   subroutine cam_grid_get_pio_decomp(this, field_lens, file_lens, dtype,     &
      ...
      if (.not. associated(this%map)) then
         ...
         if (present(field_dnames) .and. present(file_dnames)) then
            if (is_perm) then
               call cam_pio_get_decomp(iodesc, field_lens, file_lens,         &
                    dtype, this%map, field_dist_in=src_in,                    &
                    file_dist_in=dest_in, permute=permutation)
            else
               call cam_pio_get_decomp(iodesc, field_lens, file_lens,         &
                    dtype, this%map, field_dist_in=src_in,                    &
                    file_dist_in=dest_in)
            end if
   ...
   !------------------------------------------------------------------------
   !
   !  cam_grid_find_dimids: Find the dimension NetCDF IDs on <File> for
   !      this grid
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_find_dimids(this, File, dimids)
      ...
      if (size(dimids) < 1) then
         call endrun(subname//': dimids must have positive size')
      end if
      dimids = -1
      ! Check the first dimension
      ierr = pio_inq_dimid(File, trim(dimname1), dimids(1))
      if(ierr /= PIO_NOERR) then
         call endrun(subname//': '//trim(this%name)//' dimension, '//trim(dimname1)//', does not exist on file')
      end if
      if (trim(dimname1) /= trim(dimname2)) then
         ! Structured grid, find second dimid
         if (size(dimids) < 2) then
            call endrun(subname//': dimids too small for '//trim(this%name))
         end if
         ierr = pio_inq_dimid(File, trim(dimname2), dimids(2))
         if(ierr /= PIO_NOERR) then
            call endrun(subname//': '//trim(this%name)//' dimension, '//trim(dimname2)//', does not exist on file')
         end if
      end if
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_read_darray_2d_int(this, File, adims, fdims,           &
        hbuf, varid)
      ...
      type(io_desc_t),  pointer   :: iodesc
      integer                     :: ierr
      character(len=*), parameter :: subname = 'cam_grid_read_darray_2d_int'
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_read_darray_3d_int(this, File, adims, fdims,           &
        hbuf, varid)
      use pio,           only: file_desc_t, io_desc_t, pio_read_darray
      use pio,           only: PIO_INT
      use cam_pio_utils, only: cam_pio_get_decomp
      ...
      type(io_desc_t),  pointer   :: iodesc
      integer                     :: ierr
      character(len=*), parameter :: subname = 'cam_grid_read_darray_3d_int'
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_read_darray_2d_double(this, File, adims, fdims,        &
        hbuf, varid)
      use pio,           only: file_desc_t, io_desc_t, pio_read_darray
      ...
      type(io_desc_t),  pointer   :: iodesc
      integer                     :: ierr
      character(len=*), parameter :: sbnm = 'cam_grid_read_darray_2d_double'

      call cam_pio_get_decomp(iodesc, adims, fdims, PIO_DOUBLE, this%map)
      call pio_read_darray(File, varid, iodesc, hbuf, ierr)
      call cam_pio_handle_error(ierr, sbnm//': Error reading variable')
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_read_darray_3d_double(this, File, adims, fdims,        &
        hbuf, varid)
      use pio,           only: file_desc_t, io_desc_t, pio_read_darray
      use pio,           only: PIO_DOUBLE
      use cam_pio_utils, only: cam_pio_get_decomp
      ...
      type(io_desc_t),  pointer   :: iodesc
      integer                     :: ierr
      character(len=*), parameter :: sbnm = 'cam_grid_read_darray_3d_double'
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_read_darray_2d_real(this, File, adims, fdims,          &
        hbuf, varid)
      use pio,           only: file_desc_t, io_desc_t, pio_read_darray
      ...
      type(io_desc_t),  pointer   :: iodesc
      integer                     :: ierr
      character(len=*), parameter :: subname = 'cam_grid_read_darray_2d_real'
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_read_darray_3d_real(this, File, adims, fdims,          &
        hbuf, varid)
      use pio,           only: file_desc_t, io_desc_t, pio_read_darray
      use pio,           only: PIO_REAL
      use cam_pio_utils, only: cam_pio_get_decomp
      ...
      type(io_desc_t),  pointer   :: iodesc
      integer                     :: ierr
      character(len=*), parameter :: subname = 'cam_grid_read_darray_3d_real'
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_write_darray_2d_int(this, File, adims, fdims,          &
        hbuf, varid)
      ...
      type(io_desc_t),  pointer   :: iodesc
      integer                     :: ierr
      character(len=*), parameter :: subname = 'cam_grid_write_darray_2d_int'
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_write_darray_3d_int(this, File, adims, fdims,          &
        hbuf, varid)
      use pio,           only: file_desc_t, io_desc_t
      use pio,           only: pio_write_darray, PIO_INT
      use cam_pio_utils, only: cam_pio_get_decomp
      ...
      type(io_desc_t),  pointer   :: iodesc
      integer                     :: ierr
      character(len=*), parameter :: subname = 'cam_grid_write_darray_3d_int'
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_write_darray_2d_double(this, File, adims, fdims,       &
        hbuf, varid)
      use pio,           only: file_desc_t, io_desc_t
      ...
      type(io_desc_t),  pointer   :: iodesc
      integer                     :: ierr
      character(len=*), parameter :: subname = 'cam_grid_write_darray_2d_double'
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_write_darray_3d_double(this, File, adims, fdims,       &
        hbuf, varid)
      use pio,           only: file_desc_t, io_desc_t
      use pio,           only: pio_write_darray, PIO_DOUBLE
      use cam_pio_utils, only: cam_pio_get_decomp
      ...
      type(io_desc_t),  pointer   :: iodesc
      integer                     :: ierr
      character(len=*), parameter :: subname = 'cam_grid_write_darray_3d_double'
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_write_darray_2d_real(this, File, adims, fdims,         &
        hbuf, varid)
      use pio,           only: file_desc_t, io_desc_t
      ...
      type(io_desc_t),  pointer   :: iodesc
      integer                     :: ierr
      character(len=*), parameter :: subnam = 'cam_grid_write_darray_2d_real'

      call cam_pio_get_decomp(iodesc, adims, fdims, PIO_REAL, this%map)
      call pio_write_darray(File, varid, iodesc, hbuf, ierr)
      call cam_pio_handle_error(ierr, subnam//': Error writing variable')
   ...
   !
   !------------------------------------------------------------------------
   subroutine cam_grid_write_darray_3d_real(this, File, adims, fdims,         &
        hbuf, varid)
      use pio,           only: file_desc_t, io_desc_t
      use pio,           only: pio_write_darray, PIO_REAL
      use cam_pio_utils, only: cam_pio_get_decomp
      ...
      type(io_desc_t),  pointer   :: iodesc
      integer                     :: ierr
      character(len=*), parameter :: subnam = 'cam_grid_write_darray_3d_real'
   ...
   subroutine cam_grid_get_patch_mask(this, lonl, lonu, latl, latu, patch, cco)
      ...
      if (.not. associated(this%map)) then
         call endrun(subname//': Grid, '//trim(this%name)//', has no map')
      end if
      ...
      if (associated(patch%mask)) then
         if (patch%mask%num_elem() /= gridloc) then
            ...
            ! In particular, we are not zeroing the mask since multiple
            !   calls the same mask can be used for collected-column output
      ...
      if (lonl == lonu) then
         ...
         if (unstructured) then
            ...
            mindist = 181.0_r8
            if (associated(this%lat_coord%values)) then
               do latind = LBOUND(this%lat_coord%values, 1),                  &
                    UBOUND(this%lat_coord%values, 1)
            ...
            ! Get the global minimum
            dist = mindist
            call MPI_allreduce(dist, mindist, 1, mpi_real8, mpi_min,          &
                 mpicom, ierr)
            if (dist == mindist) then
   ...
   subroutine cam_grid_patch_get_decomp(this, field_lens, file_lens,          &
        dtype, iodesc, file_dest_in)
      use pio,                only: io_desc_t
      use cam_pio_utils,      only: cam_pio_get_decomp
      ...
      call cam_pio_get_decomp(iodesc, field_lens, file_lens, dtype,           &
           this%mask, file_dist_in=file_dest_in)
   ...
   subroutine cam_grid_patch_compact(this, collected_output)
      ...
      if (index > 0) then
         ...
         ! This is probably an error condition but someone else will
         !     catch it first
         dups_ok = .false.
      end if
   ...
   subroutine cam_grid_patch_write_vals(this, File, header_info)
      ...
      call cam_pio_newdecomp(iodesc, file_lens, map, pio_double)
      ...
      call cam_pio_handle_error(ierr, subname//': Error writing longitude')
      ...
      call pio_write_darray(File, vdesc, iodesc, coord, ierr)
      call cam_pio_handle_error(ierr, subname//': Error writing latitude')
