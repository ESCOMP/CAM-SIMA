! Horizontal interpolation from CAM chemistry utilities.
...
contains
  ...
  real(r8) pure function normalize_lon_right(left, right)
  ...
  ! zero.
  real(r8) pure function calculate_lon_overlap(input_left, input_right, sim_left, sim_right)
  ...
  real(r8) pure function lat_band_weight(low, high)
    ...
    lat_band_weight = sin(high*pi/180.0_r8) - sin(low*pi/180.0_r8)
  ...
  subroutine xy_interp_init(num_input_lons, num_input_lats, input_lon_radians, input_lat_radians, &
    ...
    input_lon(:) = input_lon_radians(:)/pi*180.0_r8
    input_lat(:) = input_lat_radians(:)/pi*180.0_r8
    ...
    !    lon bounds:   (lon_edge(j), lon_edge(j+1))
