...
module cam_pio_utils
   ...
   ! The tag needs the dim lengths, the dtype and map id (+ opt permutation)
   ...
   end interface cam_pio_get_var

   interface cam_pio_find_var
   ...
   end interface calc_permutation
...
!!XXgoldyXX: ^ support scam?
   ...
   subroutine cam_pio_var_info(ncid, varid, ndims, dimids, dimlens,           &
      ...
      character(len=*), parameter                :: subname = 'CAM_PIO_VAR_INFO'
      !-----------------------------------------------------------------------
      ...
      call cam_pio_handle_error(ret, subname//': Error with num dimensions')
      if (size(dimids) < ndims) then
         call endrun(subname//': dimids too small'//trim(errsuff))
      end if
      ret = PIO_inq_vardimid(ncid, varid, dimids(1:ndims))
      call cam_pio_handle_error(ret, subname//': Error with inq dim ids'//trim(errsuff))
      if (size(dimlens) < ndims) then
         call endrun(subname//': dimlens too small'//trim(errsuff))
      end if
      do i = 1, ndims
         ret = PIO_inq_dimlen(ncid, dimids(i), dimlens(i))
         call cam_pio_handle_error(ret, subname//': Error with inq dimlens')
         if (present(dimnames)) then
            ret = PIO_inq_dimname(ncid, dimids(i), dimnames(i))
            call cam_pio_handle_error(ret, subname//': Error with inq dimnames')
         end if
      end do
      if (present(unlimDimID)) then
         ret = PIO_inquire(ncid, unlimitedDimID=unlimDimID)
         call cam_pio_handle_error(ret, subname//': Error with inquire')
      end if
   ...
   subroutine cam_pio_find_var_array(ncid, var_names, found_name, varid, found)
      ...
      found = .false.
      do index = 1, num_vars
         ret = PIO_inq_varid(ncid, trim(var_names(index)), varid)
         found = (ret == PIO_NOERR)
         if (found) then
            found_name = var_names(index)
            exit
         end if
      end do
   ...
   subroutine cam_pio_get_decomp(iodesc, ldims, fdims, dtype, map,            &
      ...
      if (.not. found) then
         ...
         if (ANY(fdims == 0)) then
            ! Quick sanity check
            write(errormsg, *) 'bad fdims, ',fdims
            call endrun('cam_pio_get_decomp: '//errormsg)
         end if
   ...
   subroutine cam_pio_def_dim(File, name, size, dimid, existOK)
      use cam_abortutils, only: endrun
      use pio,            only: file_desc_t, var_desc_t
      use pio,            only: pio_inq_dimid, pio_def_dim, pio_inq_dimlen
      use pio,            only: PIO_NOERR
      use pio,            only: PIO_seterrorhandling, PIO_BCAST_ERROR
   ...
   subroutine cam_pio_get_var_2d_r8(varname, File, field, start, kount, found)
      ...
      use pio,            only: pio_inq_dimname, PIO_MAX_NAME
   ...
   logical function cam_pio_fileexists(fname)
      ...
      use pio,            only: pio_seterrorhandling, PIO_BCAST_ERROR
      use pio,            only : pio_closefile, PIO_NOWRITE
   ...
   subroutine find_dump_filename(fieldname, filename)
      ...
      filename = trim(fieldname)//'_dump_1.nc'
      fnum = 1
      do while (cam_pio_fileexists(trim(filename)))
         fnum = fnum + 1
         write(filename, '(2a,i0,a)') trim(fieldname), '_dump_', fnum, '.nc'
      end do
   ...
   subroutine dump_field_2d_d(fieldname, dim1b, dim1e, dim2b, dim2e, field,   &
      ...
      real(r8), target,   intent(in)        :: field(dim1b:dim1e,dim2b:dim2e)
      ...
      allocate(ldof((dim2e - dim2b + 1) * (dim1e - dim1b + 1)))
      m = 0
      do j = dim2b, dim2e
         do i = dim1b, dim1e
            m = m + 1
   ...
   subroutine dump_field_3d_d(fieldname, dim1b, dim1e, dim2b, dim2e,          &
      ...
      real(r8), target,   intent(in)        :: field(dim1b:dim1e,dim2b:dim2e,dim3b:dim3e)
      logical,  optional, intent(in)        :: compute_maxdim_in
      real(r8), optional, intent(in)        :: fill_value
      ...
      ! Compute the variable decomposition
      lsize = product(dimsizes(1:3))
      allocate(ldof((dim3e-dim3b+1) * (dim2e-dim2b+1) * (dim1e-dim1b+1)))
      m = 0
      do k = dim3b, dim3e
         do j = dim2b, dim2e
            do i = dim1b, dim1e
   ...
   subroutine dump_field_4d_d(fieldname, dim1b, dim1e, dim2b, dim2e,          &
      ...
      integer,            intent(in)        :: dim3b
      integer,            intent(in)        :: dim3e
      integer,            intent(in)        :: dim4b
      integer,            intent(in)        :: dim4e
      real(r8), target,   intent(in)        :: field(dim1b:dim1e,dim2b:dim2e,dim3b:dim3e,dim4b:dim4e)
      logical,  optional, intent(in)        :: compute_maxdim_in
      real(r8), optional, intent(in)        :: fill_value
      ...
      ! Compute the variable decomposition
      lsize = product(dimsizes(1:4))
      allocate(ldof((dim4e-dim4b+1) * (dim3e-dim3b+1) * (dim2e-dim2b+1) * (dim1e-dim1b+1)))
      m = 0
      do l = dim4b, dim4e
         do k = dim3b, dim3e
            do j = dim2b, dim2e
   ...
   subroutine dump_field_6d_d(fieldname, dimbs, dimes, field,                 &
      ...
      if (compute_maxdim) then
         ...
         do i1 = 1, 6
            dimsizes(i1) = dimes(i1) - dimbs(i1) + 1
         end do
      end if
      dimsizes(7) = npes
      do ind = 1, 7
         write(filename, '(a,i0)') 'dim', ind
         call cam_pio_def_dim(file, trim(filename), dimsizes(ind), dimids(ind))
      end do
      call cam_pio_def_dim(file, 'bounds', size(bounds, 1), bnddimid)
      ! Define the variables
      call cam_pio_def_var(file, 'field_bounds', pio_int,                     &
      ...
      lsize = 1
      do ind = 1, 6
         lsize = lsize * (dimes(ind) - dimbs(ind) + 1)
      end do
      allocate(ldof(lsize))
      ind = 0
      do i6 = dimbs(6), dimes(6)
         i(6) = i6
         do i5 = dimbs(5), dimes(5)
            i(5) = i5
            do i4 = dimbs(4), dimes(4)
               i(4) = i4
               do i3 = dimbs(3), dimes(3)
                  i(3) = i3
                  do i2 = dimbs(2), dimes(2)
                     i(2) = i2
                     do i1 = dimbs(1), dimes(1)
                        i(1) = i1
                        ind = ind + 1
                        ldof(ind) = iam
                        do j = 6, 1, -1
                           ldof(ind) = (ldof(ind) * dimsizes(j)) + (i(j) - dimbs(j))
                        end do
                     end do
                  end do
               end do
      ...
      ! Compute the bounds decomposition and write field bounds
      do ind = 1, 6
         bounds(2*ind - 1) = dimbs(ind)
         bounds(2*ind)     = dimes(ind)
      end do
      bounds(13) = 1
      bounds(14) = npes
      dimsizes(1) = size(bounds, 1)
      dimsizes(2) = npes
      allocate(ldof(size(bounds, 1)))
      do ind = 1, size(bounds, 1)
         ldof(ind) = (iam * size(bounds, 1)) + ind
      end do
      call pio_initdecomp(pio_subsystem, PIO_INT, dimsizes(1:2), ldof, iodesc)
      call pio_write_darray(file, bnddesc, iodesc, bounds, ierr, -900)
      call pio_freedecomp(file, iodesc)
   ...
   end subroutine dump_field_6d_d
