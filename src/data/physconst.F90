module physconst
   ...
   real(kind_phys), public, parameter :: avogad      = shr_const_avogad     ! Avogadro's number (molecules kmole-1)
   real(kind_phys), public, parameter :: boltz       = shr_const_boltz      ! Boltzman's constant (J K-1 molecule-1)
   real(kind_phys), public, parameter :: cday        = shr_const_cday       ! sec in calendar day (seconds)
   real(kind_phys), public, parameter :: cpliq       = shr_const_cpfw       ! specific heat of fresh h2o (J K-1 kg-1)
   real(kind_phys), public, parameter :: cpice       = shr_const_cpice      ! specific heat of ice (J K-1 kg-1)
   real(kind_phys), public, parameter :: karman      = shr_const_karman     ! Von Karman constant
   real(kind_phys), public, parameter :: latice      = shr_const_latice     ! Latent heat of fusion (J kg-1)
   real(kind_phys), public, parameter :: latvap      = shr_const_latvap     ! Latent heat of vaporization (J kg-1)
   real(kind_phys), public, parameter :: pi          = shr_const_pi         ! 3.14...
#ifdef planet_mars
   real(kind_phys), public, parameter :: pstd        = 6.0E1_kind_phys             ! Standard pressure (Pascals)
#else
   real(kind_phys), public, parameter :: pstd        = shr_const_pstd       ! Standard pressure (Pascals)
...
#endif
   real(kind_phys), public, parameter :: r_universal = shr_const_rgas       ! Universal gas constant (J K-1 kmol-1)
   real(kind_phys), public, parameter :: rhoh2o      = shr_const_rhofw      ! Density of liquid water at STP (kg m-3)
   real(kind_phys), public, parameter :: spval       = shr_const_spval      !special value
   real(kind_phys), public, parameter :: stebol      = shr_const_stebol     ! Stefan-Boltzmann's constant (W m-2 K-4)
   real(kind_phys), public, parameter :: h2otrip     = shr_const_tktrip     ! Triple point temperature of water (K)
   ...
   real(kind_phys), public, protected :: mwh2o       =  shr_const_mwwv     ! molecular weight h2o
   real(kind_phys), public, protected :: mwdry       =  shr_const_mwdair   ! molecular weight dry air

   ! modifiable physical constants for  other planets (including aquaplanet)
   real(kind_phys), public, protected :: gravit  = shr_const_g            ! gravitational acceleration (m s-2)
   real(kind_phys), public, protected :: sday    = shr_const_sday         ! sec in sidereal day (seconds)
   real(kind_phys), public, protected :: cpwv    = shr_const_cpwv         ! specific heat of water vapor (J K-1 kg-1)
   real(kind_phys), public, protected :: cpair   = shr_const_cpdair       ! specific heat of dry air (J K-1 kg-1)
   real(kind_phys), public, protected :: rearth  = shr_const_rearth       ! radius of earth (m)
   real(kind_phys), public, protected :: tmelt   = shr_const_tkfrz        ! Freezing point of water (K)

   !-----  Variables below here are derived from those above -----------------

   real(kind_phys), public, protected :: rga        = 1._kind_phys/shr_const_g         ! reciprocal of gravit (s2 m-1)
   real(kind_phys), public, protected :: ra         = 1._kind_phys/shr_const_rearth    ! reciprocal of earth radius (m-1)
   real(kind_phys), public, protected :: omega      = shr_const_omega           ! earth rot (rad sec-1)
   real(kind_phys), public, protected :: rh2o       = shr_const_rwv             ! Water vapor gas constant (J K-1 kg-1)
   real(kind_phys), public, protected :: rair       = shr_const_rdair           ! Dry air gas constant     (J K-1 kg-1)
   real(kind_phys), public, protected :: epsilo     = shr_const_mwwv/shr_const_mwdair   ! ratio of h2o to dry air molecular weights
   real(kind_phys), public, protected :: zvir       = shr_const_zvir            ! (rh2o/rair) - 1
   real(kind_phys), public, protected :: cpvir      = shr_const_cpvir           ! CPWV/CPDAIR - 1.0
   real(kind_phys), public, protected :: rhodair    = shr_const_rhodair         ! density of dry air at STP (kg m-3)
   real(kind_phys), public, protected :: cappa      = (shr_const_rgas/shr_const_mwdair)/shr_const_cpdair  ! R/Cp
   real(kind_phys), public, protected :: ez                                     ! Coriolis expansion coeff -> omega/sqrt(0.375)
   real(kind_phys), public, protected :: Cpd_on_Cpv = shr_const_cpdair/shr_const_cpwv
