...
module cam_thermo
   ...
   ! kmvis: molecular viscosity      kg m-1 s-1
   real(kind_phys), public, protected, allocatable :: kmvis(:,:)
   ...
   interface get_exner
      ...
      module procedure get_exner_2hd
...
CONTAINS
   ...
   subroutine get_enthalpy_1hd(tracer_mass, temp, dp_dry,               &
      ...
      real(kind_phys),          intent(in)  :: tracer_mass(:,:,:)
      ! temp: temperature
      ...
      real(kind_phys),          intent(out) :: enthalpy(:,:)
      !
      ...
      if (dry_air_species_num == 0) then
         ...
         if (present(active_species_idx_dycore)) then
            call get_cp_dry(tracer_mass, active_species_idx_dycore,           &
                 enthalpy, fact=1.0_kind_phys/dp_dry(:,:))
         else
   ...
   subroutine get_virtual_temp_1hd(tracer, T_v, temp, dp_dry, sum_q,          &
      ...
      real(kind_phys),           intent(out) :: T_v(:, :)
      ! temp: temperature
      real(kind_phys), optional, intent(in)  :: temp(:, :)
      ! dp_dry: dry pressure level thickness
      real(kind_phys), optional, intent(in)  :: dp_dry(:, :)
      ! sum_q: sum tracer
      real(kind_phys), optional, intent(out) :: sum_q(:, :)
      !
      ! array of indicies for index of thermodynamic active species in
      !    dycore tracer array (if different from physics index)
      !
   ...
   subroutine get_sum_species_1hd(tracer, active_species_idx,                 &
      ...
      if (present(dp_dry)) then
         factor_loc = 1.0_kind_phys / dp_dry(:,:)
      else
         factor_loc = 1.0_kind_phys
      end if
   ...
   subroutine get_dp_1hd(tracer, mixing_ratio, active_species_idx, dp_dry, dp, ps, ptop)
     ...
     if (mixing_ratio == DRY_MIXING_RATIO) then
       ...
       do qdx = dry_air_species_num + 1, thermodynamic_active_species_num
        ...
        call endrun(subname//'unrecognized input ('//to_str(mixing_ratio)//') for mixing_ratio')
     end if
     if (present(ps)) then
   ...
   subroutine get_pmid_from_dpdry_1hd(tracer, mixing_ratio, active_species_idx, dp_dry, ptop, pmid, pint, dp)
     ...
     real(kind_phys), optional, intent(out) :: pint(:,:)                ! half-level pressure
     real(kind_phys), optional, intent(out) :: dp(:,:)                  ! presure level thickness

     real(kind_phys) :: dp_local(SIZE(tracer, 1), SIZE(tracer, 2))      ! local pressure level thickness
     real(kind_phys) :: pint_local(SIZE(tracer, 1), SIZE(tracer, 2) + 1)! local interface pressure
     integer  :: kdx
   ...
   subroutine get_pmid_from_dp_1hd(dp, ptop, pmid, pint)
     ...
     real(kind_phys), intent(in)            :: dp(:,:)     ! pressure level thickness
     real(kind_phys), intent(in)            :: ptop        ! pressure at model top
     real(kind_phys), intent(out)           :: pmid(:,:)   ! mid (full) level pressure
     real(kind_phys), optional, intent(out) :: pint(:,:)   ! pressure at interfaces (half levels)

     real(kind_phys) :: pint_local(SIZE(dp, 1), SIZE(dp,2) + 1)
     integer  :: kdx
     ...
     if (dycore_gz_log_calc) then
       ...
       do kdx = 1, SIZE(dp, 2)
         pmid(:, kdx) = 0.5_kind_phys * (pint_local(:, kdx) + pint_local(:, kdx + 1))
       end do
     end if
     if (present(pint)) pint=pint_local
   ...
   subroutine get_exner_1hd(tracer, mixing_ratio, active_species_idx, dp_dry, ptop, p00, inv_exner, exner, poverp0)
     ...
     real(kind_phys) :: pmid(SIZE(tracer, 1), SIZE(tracer, 2))
     real(kind_phys) :: kappa_dry(SIZE(tracer, 1), SIZE(tracer, 2))
     ...
     if (mixing_ratio == DRY_MIXING_RATIO) then
       ...
       call get_kappa_dry(tracer, active_species_idx, kappa_dry, 1.0_kind_phys / dp_dry)
     else
       call endrun(subname//'unrecognized input ('//to_str(mixing_ratio)//') for mixing_ratio')
     end if
     if (inv_exner) then
   ...
   subroutine get_exner_2hd(tracer, mixing_ratio, active_species_idx, dp_dry, ptop, p00, inv_exner, exner, poverp0)
     ...
     do jdx = 1, size(dp_dry, 2)
        if (present(poverp0)) then
           call get_exner(tracer(:, jdx, :, :), mixing_ratio, active_species_idx, &
              dp_dry(:, jdx, :), ptop, p00, inv_exner, exner(:, jdx, :), poverp0=poverp0(:, jdx, :))
        else
           call get_exner(tracer(:, jdx, :, :), mixing_ratio, active_species_idx, &
   ...
   subroutine get_virtual_theta_1hd(tracer, mixing_ratio, active_species_idx, dp_dry, ptop, p00, temp, theta_v)
     ...
     real(kind_phys), intent(out) :: theta_v(:,:)                       ! virtual potential temperature

     real(kind_phys) :: iexner(SIZE(tracer, 1), SIZE(tracer, 2))
   ...
   subroutine get_gz_from_dp_dry_ptop_temp_1hd(tracer, mixing_ratio, active_species_idx, &
     ...
     real(kind_phys), intent(out) :: gz(:,:)                            ! geopotential
     real(kind_phys), optional, intent(out) :: pmid(:,:)                ! mid-level pressure
     real(kind_phys), optional, intent(out) :: dp(:,:)                  ! pressure level thickness
     real(kind_phys), optional, intent(out) :: t_v(:,:)                 ! virtual temperature
     ...
     if (mixing_ratio == DRY_MIXING_RATIO) then
       ...
       call get_R_dry(tracer,active_species_idx, R_dry, fact=1.0_kind_phys / dp_dry)
     else
       call endrun(subname//'unrecognized input ('//to_str(mixing_ratio)//') for mixing_ratio')
     end if
   ...
   subroutine get_gz_given_dp_Tv_Rdry_1hd(dp, T_v, R_dry, phis, ptop, gz, pmid)
     use physics_types,          only: dycore_gz_log_calc
     real(kind_phys), intent(in)  :: dp   (:,:)                       ! pressure level thickness
     real(kind_phys), intent(in)  :: T_v  (:,:)                       ! virtual temperature
     real(kind_phys), intent(in)  :: R_dry(:,:)                       ! R dry
     real(kind_phys), intent(in)  :: phis (:)                         ! surface geopotential
     ...
     real(kind_phys), dimension(SIZE(dp, 1), SIZE(dp, 2))      :: pmid_local
     real(kind_phys), dimension(SIZE(dp, 1), SIZE(dp, 2) + 1)  :: pint
     real(kind_phys), dimension(SIZE(dp, 1))                   :: gzh, Rdry_tv
     integer :: kdx
     ...
     if (dycore_gz_log_calc) then
       do kdx = SIZE(dp, 2), 1, -1
         ...
         gz(:, kdx) = gzh(:) + Rdry_tv(:) * (1.0_kind_phys - pint(:, kdx) / pmid_local(:, kdx))
         gzh(:)  = gzh(:) + Rdry_tv(:) * (log(pint(:, kdx + 1)) - log(pint(:, kdx)))
       end do
     else
       do kdx = SIZE(dp,2), 1, -1
         Rdry_tv(:) = R_dry(:,kdx) * T_v(:, kdx)
         gz(:,kdx) = gzh(:) + Rdry_tv(:) * 0.5_kind_phys * dp(:, kdx) / pmid_local(:, kdx)
   ...
   subroutine get_Richardson_number_1hd(tracer,mixing_ratio, active_species_idx, dp_dry, ptop, &
     ...
     real(kind_phys), intent(in)  :: p00                                  ! reference pressure for Exner pressure (usually 1000hPa)
     real(kind_phys), intent(in)  :: temp(:,:)                            ! temperature
     real(kind_phys), intent(in)  :: v(:,:,:)                             ! velocity components
     real(kind_phys), intent(out) :: Richardson_number(:,:)
     real(kind_phys), optional, intent(out) :: pmid(:,:)
     real(kind_phys), optional, intent(out) :: dp(:,:)
     ...
     Richardson_number(:, 1)                   = 0.0_kind_phys
     Richardson_number(:, SIZE(tracer, 2) + 1) = 0.0_kind_phys
   ...
   subroutine get_dp_ref_1hd(hyai, hybi, ps0, phis, dp_ref, ps_ref)
     ...
     real(kind_phys), intent(in)           :: hyai(:)
     real(kind_phys), intent(in)           :: hybi(:)
     real(kind_phys), intent(in)           :: ps0
     real(kind_phys), intent(in)           :: phis(:)
     real(kind_phys), intent(out)          :: dp_ref(:,:)
     real(kind_phys), intent(out)          :: ps_ref(:)
     integer :: kdx
     !
   ...
   subroutine get_rho_dry_1hd(tracer, temp, ptop, dp_dry, tracer_mass, rho_dry, rhoi_dry, &
     ...
     ! local vars
     integer :: idx, kdx
     real(kind_phys),  dimension(SIZE(tracer, 1), SIZE(tracer, 2))        :: pmid
     real(kind_phys),  dimension(SIZE(tracer, 1), SIZE(tracer, 2) + 1)    :: pint
     ...
     if (present(rhoi_dry)) then
       ...
       do kdx = 2, SIZE(tracer, 2) + 1
         rhoi_dry(:, kdx) = 0.5_kind_phys * (temp(:, kdx) + temp(:, kdx - 1))!could be more accurate!
       ...
       !
       kdx=1
       rhoi_dry(:, kdx) = 1.5_kind_phys * (temp(:, kdx) - 0.5_kind_phys * temp(:, kdx + 1))
       rhoi_dry(:, kdx) = pint(:, kdx) / (rhoi_dry(:, kdx) * R_dry(:, kdx)) !ideal gas law for dry air
       deallocate(R_dry)
     end if
     ...
     if (dry_air_species_num==0) then
       ...
       if (get_at_interfaces) then
           do kdx = 2, SIZE(sponge_factor, 1)
             do idx = 1, SIZE(tracer, 1)
               temp_local   = 0.5_kind_phys * (temp(idx, kdx) + temp(idx, kdx - 1))
           ...
           ! extrapolate top level value
           !
           kmvis(1:SIZE(tracer, 1), 1) = 1.5_kind_phys * kmvis(1:SIZE(tracer, 1), 2) - 0.5_kind_phys * kmvis(1:SIZE(tracer, 1), 3)
           kmcnd(1:SIZE(tracer, 1), 1) = 1.5_kind_phys * kmcnd(1:SIZE(tracer, 1), 2) - 0.5_kind_phys * kmcnd(1:SIZE(tracer, 1), 3)
       ...
       if (get_at_interfaces) then
         do kdx = 2, SIZE(sponge_factor, 1)
           do idx = 1, SIZE(tracer, 1)
             kmvis(idx, kdx) = 0.0_kind_phys
             kmcnd(idx, kdx) = 0.0_kind_phys
             residual = 1.0_kind_phys
             do icnst = 1, dry_air_species_num
               ispecies = idx_local(icnst)
               mm       = 0.5_kind_phys * (tracer(idx, kdx, ispecies) * factor(idx, kdx) + tracer(idx, kdx - 1, ispecies) * factor(idx, kdx-1))
             ...
             temp_local = 0.5_kind_phys * (temp(idx, kdx - 1) + temp(idx, kdx))
         ...
         do idx = 1, SIZE(tracer, 1)
           ...
           kmcnd(idx, 1) = 1.5_kind_phys * kmcnd(idx, 2) - .5_kind_phys * kmcnd(idx, 3)
   ...
   subroutine get_molecular_diff_coef_reference(tref,press,sponge_factor,kmvis_ref,kmcnd_ref,rho_ref)
     ...
     real(kind_phys), intent(in)           :: press(:)             !pressure
     real(kind_phys), intent(in)           :: sponge_factor(:)     !multiply kmvis and kmcnd with sponge_factor (for sponge layer)
     real(kind_phys), intent(out)          :: kmvis_ref(:)         !reference molecular diffusion coefficient
     real(kind_phys), intent(out)          :: kmcnd_ref(:)         !reference thermal conductivity coefficient
     real(kind_phys), intent(out)          :: rho_ref(:)           !reference density
   ...
   subroutine cam_thermo_calc_kappav_2hd(tracer, kappav, cpv)
      ...
      ! Dummy arguments
      real(kind_phys),           intent(in)  :: tracer(:, :, :, :)
      real(kind_phys),           intent(out) :: kappav(:, :, :)
      real(kind_phys), optional, intent(out) :: cpv(:, :, :)

      ! Local variables
      real(kind_phys) :: rgas_var(SIZE(tracer, 1), SIZE(tracer, 2), SIZE(tracer, 3))
      real(kind_phys) :: cp_var(SIZE(tracer, 1), SIZE(tracer, 2), SIZE(tracer, 3))
      integer :: ind, jnd, knd
   ...
   subroutine get_hydrostatic_energy_1hd(tracer, moist_mixing_ratio, pdel_in, &
      ...
      real(kind_phys), intent(out), optional :: H2O(:)
      ...
      real(kind_phys), intent(out), optional :: liq(:)
      ! ice: vertically integrated ice
      real(kind_phys), intent(out), optional :: ice(:)
