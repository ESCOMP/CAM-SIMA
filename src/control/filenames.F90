...
!==============================================================================
   ...
   character(len=cl) function get_dir(filepath)
      ...
      ! local variables
      ! filenameposition: Character position for last character of directory
      integer :: filenameposition
      !------------------------------------------------------------------------
   ...
   character(len=cl) function interpret_filename_spec(filename_spec, number,  &
        prev, case, yr_spec, mon_spec, day_spec, sec_spec)
      ...
      if (index(trim(filename_spec)," ") /= 0)then
         call endrun (subname//': filename specifier can not contain a space:'//trim(filename_spec))
      end if
      !
      ...
      !
      ! Go through each character in the filename specifyer and
      !   interpret if special string
      !
      i = 1
         ...
         if (filename_spec(i:i) == "%")then
            ...
            case('t')   ! number
               if (.not. present(number))then
                  write(iulog,*) subname, ': number needed in filename_spec', &
                       ', but not provided to subroutine'
                  write(iulog,*) 'filename_spec = ', trim(filename_spec)
                  call endrun(subname//'filename_spec = '//trim(filename_spec))
               end if
               if (number > 999) then
                  format = '(i4.4)'
                  if (number > 9999) then
                     write(errmsg, '(2a,i0)') subname,                        &
                          ': number is too large: ', number
            ...
            case('%')   ! percent character
               string = "%"
            case default
               call endrun(subname//': Invalid expansion character: '//filename_spec(i:i))
            end select
            !
