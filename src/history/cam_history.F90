module cam_history
   ...
   use solar_parms_data,    only: kp=>solar_parms_kp
   use solar_parms_data,    only: ap=>solar_parms_ap
   use solar_parms_data,    only: f107=>solar_parms_f107
   use solar_parms_data,    only: f107a=>solar_parms_f107a
   use solar_parms_data,    only: f107p=>solar_parms_f107p
   ...
   type master_entry
      ...
      ! meridional and zonal fields are for vector field pairs
      character(len=max_fieldname_len) :: meridional_field = ''
      character(len=max_fieldname_len) :: zonal_field = ''
      ! avgflag represents the default averaging for each file
      character(len=1)  :: avgflag(pfiles)
      character(len=CS) :: time_op(pfiles)   ! time operator (e.g. max, min, avg)
      logical           :: act_somefile      ! Field is active on some file
      logical           :: actflag(pfiles)   ! Per file active/inactive flag
      integer           :: hfileindx(pfiles) ! This field's index on each file
      type(master_entry), pointer :: next_entry => null() ! Next master entry
   contains
      ! time_op is a description of the avgflag property
      procedure :: time_op => AvgflagToString
   end type master_entry

   type (master_entry), pointer :: masterlinkedlist => null() ! top of master field linkedlist
   ...
   ! per file sampling frequency (0=monthly avg)

   integer :: idx                      ! for module variable initialization
   integer :: hist_freq(pfiles) = (/0, (-24, idx=2,pfiles)/)  ! history write frequency (0 = monthly)
   integer :: mfilt(pfiles) = 30       ! number of time samples per file
   integer :: nfils(pfiles)            ! Array of no. of files on current h-file
   integer :: ndens(pfiles) = 2        ! packing density (double (1) or real (2))
   integer :: ncprec(pfiles) = -999    ! netcdf packing parameter based on ndens
   ...
   ! Parameters for interpolated output files
   logical, public       :: interpolate_output(pfiles) = .false.
   ! The last two history files are not supported for interpolation
   type(interp_info_t)   :: interpolate_info(pfiles - 2)
   ...
   public :: hist_init_files           ! Initialization
   ...
   public :: get_pfiles                ! Return the number of files being used
...
CONTAINS
   ...
   subroutine hist_init_files(model_doi_url_in, caseid_in, ctitle_in)
      !
      !-----------------------------------------------------------------------
      !
...
9000        format(i5, 1x, a32, 1x, a16, 1x, i4, 1x, a1, 2x, a)
   ...
   subroutine history_readnl(nlfile)
      ...
      namelist /cam_history_nl/ ndens, hist_freq, mfilt,                      &
           avgflag_perfile, empty_hfiles, lcltod_start, lcltod_stop,          &
           inithist, inithist_all,                                            &
           fincl1lonlat, fincl2lonlat, fincl3lonlat, fincl4lonlat,            &
           fincl5lonlat, fincl6lonlat, fincl7lonlat, fincl8lonlat,            &
           fincl9lonlat, fincl10lonlat,                                       &
   ...
   character(len=max_string_len) function get_hfilepath( file )
      !
      !-----------------------------------------------------------------------
      !
      ! Purpose: Return full filepath of history file for given file number
      ...
      get_hfilepath = cpath(file)
   end function get_hfilepath

   !#######################################################################

   character(len=max_string_len) function get_hist_restart_filepath( file )
      !
      !-----------------------------------------------------------------------
      !
      ! Purpose: Return full filepath of restart file for given file number
      ! This allows public read access to the filenames without making
      ! the filenames public data.
      !
      !-----------------------------------------------------------------------
      ...
      get_hist_restart_filepath = hrestpath( file )
   end function get_hist_restart_filepath

   !#######################################################################

   integer function get_pfiles( )
      !
      !-----------------------------------------------------------------------
      !
      ! Purpose: Return the number of files being used.
      ! This allows public read access to the number of files without making
      ! pfiles public data.
      !
      !-----------------------------------------------------------------------
      !
      get_pfiles = pfiles
   end function get_pfiles
...
300               format('WSHIST: writing history restart ',i3,' to hr-file ', &
            ...
            if ( (nfils(fil_idx) == 0) .or. &
               ...
               if(restart) then
                  ...
                  fname = interpret_filename_spec( hfilename_spec(fil_idx), number=(t-1), &
                       prev=prev )
               end if
               !
