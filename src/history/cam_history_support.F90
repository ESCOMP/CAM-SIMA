module cam_history_support
   ...
   type, public :: history_patch_t
      ...
      ! lon_axis_name and lat_axis_name only used if collected_output = .false.
   ...
   type, public :: field_info
      ...
      ! flag_xyfill: non-applicable xy points flagged with fillvalue
      logical :: flag_xyfill = .false.
      ! is_succol: .true. iff field output as subcol
      logical :: is_subcol = .false.
      ! mdims: indicies into hist_coords list
      integer, pointer :: mdims(:) => NULL()
      ! file_shape: shape of field on file
      integer, pointer :: file_shape(:) => NULL()
      ! field_shape: shape of field on file
      integer, pointer :: field_shape(:) => NULL()
      ! fillvalue: fillvalue for this variable
      real(r8) :: fillvalue = hist_default_fillvalue
      ! decomp_type:  type of decomposition (e.g., physics or dynamics)
      integer :: decomp_type
   ...
   type, public:: active_entry
      ...
      type(var_desc_t) :: mdtid          ! varid for timestep
      type(var_desc_t) :: ndbaseid       ! varid for base day
      type(var_desc_t) :: nsbaseid       ! varid for base seconds of base day
      type(var_desc_t) :: nbdateid       ! varid for base date
      type(var_desc_t) :: nbsecid        ! varid for base seconds of base date
      type(var_desc_t) :: ndcurid        ! varid for current day
      type(var_desc_t) :: nscurid        ! varid for current seconds of current day
      type(var_desc_t) :: dateid         ! varid for current date
      type(var_desc_t) :: co2vmrid       ! varid for co2 volume mixing ratio
      type(var_desc_t) :: ch4vmrid       ! varid for ch4 volume mixing ratio
      type(var_desc_t) :: n2ovmrid       ! varid for n2o volume mixing ratio
      type(var_desc_t) :: f11vmrid       ! varid for f11 volume mixing ratio
      type(var_desc_t) :: f12vmrid       ! varid for f12 volume mixing ratio
      type(var_desc_t) :: sol_tsiid      ! varid for total solar irradiance (W/m2)
      type(var_desc_t) :: datesecid      ! varid for curent seconds of current date
#if ( defined BFB_CAM_SCAM_IOP )
      type(var_desc_t) :: bdateid        ! varid for base date
      type(var_desc_t) :: tsecid         ! varid for curent seconds of current date
#endif
      type(var_desc_t) :: nstephid       ! varid for current timestep
      type(var_desc_t) :: timeid         ! varid for time
      type(var_desc_t) :: tbndid         ! varid for time_bnds
      type(var_desc_t) :: date_writtenid ! varid for date time sample written
      type(var_desc_t) :: time_writtenid ! varid for time time sample written
      type(var_desc_t) :: f107id         ! varid for f107
      type(var_desc_t) :: f107aid        ! varid for f107a
      type(var_desc_t) :: f107pid        ! varid for f107p
      type(var_desc_t) :: kpid           ! varid for kp
      type(var_desc_t) :: apid           ! varid for ap
      type(var_desc_t) :: byimfid        ! varid IMF BY
      type(var_desc_t) :: bzimfid        ! varid IMF BZ
      type(var_desc_t) :: swvelid        ! varid solar wind velocity
      type(var_desc_t) :: swdenid        ! varid solar wind density
   ...
   !---------------------------------------------------------------------------
   !
   !  hist_interp_info_t: Information for lat/lon interpolated history output
   !
   !---------------------------------------------------------------------------
   ...
   end interface check_hist_coord
...
CONTAINS
   ...
   subroutine interp_reset(this)
      type(hist_interp_info_t), intent(inout) :: this
   ...
   subroutine field_info_get_bounds(this, dim, dbeg, dend)

      ! Dummy arguments
      class(field_info)                :: this
      integer,           intent(in)    :: dim
      integer,           intent(out)   :: dbeg
      integer,           intent(out)   :: dend

      select case(dim)
      case (1)
         dbeg = this%dbegdim1
         dend select= this%enddim1
      case (2)
         dbeg = this%dbegdim2
         dend = this%enddim2
      case (3)
   ...
   subroutine history_patch_write_attrs(this, File)
      ...
      if (associated(this%header_info)) then
         ...
         if (size(this%header_info) /= num_patches) then
            write(errormsg, '(2a,2(i0,a))') 'Size mismatch between ',         &
                 'header_info (', size(this%header_info), ') and patches (',  &
                 num_patches, ')'
      ...
      do i = 1, num_patches
         ...
         if (len_trim(col_name) > 0) then
            ...
            if (unstruct .and. (.not. col_only)) then
               ...
               col_name = trim(col_name)//'_'//trim(this%lon_axis_name)//     &
                    '_'//trim(this%lat_axis_name)
            end if
            call cam_pio_def_dim(File, trim(col_name), temp1, dimid1,         &
                 existOK=.false.)
            ...
            call cam_pio_def_dim(File, trim(lat_name), temp1, dimid1,         &
                 existOK=.true.)
            latid = dimid1
            ...
            call cam_pio_def_dim(File, trim(lon_name), temp2, dimid2,         &
                 existOK=.true.)
            lonid = dimid2
         ...
         if (unstruct .and. (.not. col_only)) then
            ...
            lat_name = trim(lat_name)//'_'//trim(this%lon_axis_name)//'_'//   &
                 trim(this%lat_axis_name)
         end if
         ...
         if (len_trim(temp_str) > 0) then
            ...
            call cam_pio_handle_error(ierr, subname//                         &
                 ': Unable to define long_name')
         end if
         ...
         if (unstruct .and. (.not. col_only)) then
            ...
            lon_name = trim(lon_name)//'_'//trim(this%lon_axis_name)//'_'//   &
                 trim(this%lat_axis_name)
         end if
   ...
   subroutine history_patch_write_vals(this, File)
      ...
      if (.not. associated(this%header_info)) then
         ...
         write(errormsg, '(2a)') 'No header info for ',                       &
              trim(this%namelist_entry)
         call endrun('history_patch_write_vals: '//trim(errormsg))
      end if
   ...
   subroutine history_patch_field_name(this, name)
      ...
      if (.not. this%collected_output) then
         ...
         name = trim(name)//'_'//trim(this%lon_axis_name)//'_'//              &
              trim(this%lat_axis_name)
      end if
   ...
   integer function history_patch_num_hdims(this, gridid)
      ...
      if (associated(this%header_info)) then
         ! Make sure header_info is the right size
         if (size(this%header_info) /= num_patches) then
            write(errormsg, '(a,2(i0,a))')                                    &
                 'Size mismatch between header_info (',                       &
                 size(this%header_info), ') and patches (', num_patches, ')'
            call endrun('history_patch_num_hdims: '//trim(errormsg))
         end if
      else
         write(errormsg, *) 'No header info for patch, ',                     &
              trim(this%namelist_entry)
         call endrun('history_patch_num_hdims: '//trim(errormsg))
      end if
      ...
      if (associated(this%header_info)) then
         ...
         if (size(this%header_info) /= num_patches) then
            write(errormsg, '(a,2(i0,a))')                                    &
                 'Size mismatch between header_info (',                       &
                 size(this%header_info), ') and patches (', num_patches, ')'
            call endrun('history_patch_get_var_data: '//trim(errormsg))
         end if
      else
         write(errormsg, *) 'No header info for patch, ',                     &
              trim(this%namelist_entry)
   ...
   subroutine history_patch_write_var(this, File, gridid, adims, dtype,       &
        hbuf, varid)
      ...
      character(len=*), parameter :: subname = 'history_patch_write_var: '
      ...
      if (size(adims) == 2) then
         ...
         call endrun(subname//"adims must be rank 2 or 3")
      end if
