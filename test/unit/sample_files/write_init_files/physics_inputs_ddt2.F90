    ...
    do suite_idx = 1, size(suite_names, 1)
      ...
      do req_idx = 1, size(ccpp_required_data, 1)
        ...
        if (trim(phys_var_stdnames(name_idx)) == 'latitude') then
          call read_field(file, input_var_names(:,name_idx), timestep, phys_state%latitude)
        end if

        if (trim(phys_var_stdnames(name_idx)) == 'longitude') then
          call read_field(file, input_var_names(:,name_idx), timestep, phys_state%longitude)
        end if

        if (trim(phys_var_stdnames(name_idx)) == 'eastward_wind') then
          call read_field(file, input_var_names(:,name_idx), 'lev', timestep, phys_state%wind%u)
        end if

        if (trim(phys_var_stdnames(name_idx)) == 'northward_wind') then
          call read_field(file, input_var_names(:,name_idx), 'lev', timestep, phys_state%wind%v)
        end if

        if (trim(phys_var_stdnames(name_idx)) == 'horizontal_dimension') then
          call read_field(file, input_var_names(:,name_idx), timestep, phys_state%ncol)
        end if

        if (trim(phys_var_stdnames(name_idx)) == 'vertical_layer_dimension') then
          call read_field(file, input_var_names(:,name_idx), timestep, phys_state%pver)
        end if
