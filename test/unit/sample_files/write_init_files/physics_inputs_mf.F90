...
CONTAINS
  ...
  subroutine physics_read_data(file, suite_names, timestep)
    ...
    use physics_types,        only: phys_state, psurf_ref
    ...
    do suite_idx = 1, size(suite_names, 1)
      ...
      do req_idx = 1, size(ccpp_required_data, 1)
        ...
        if (trim(phys_var_stdnames(name_idx)) == 'surface_reference_air_pressure') then
          call read_field(file, input_var_names(:,name_idx), timestep, psurf_ref)
        end if
