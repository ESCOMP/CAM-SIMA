...
contains
   ...
   subroutine restart_physics_write(file, grid_id, errmsg, errflg)
      ...
      call cam_grid_write_dist_array(file, grid_decomp, dims, field_shape, phys_state%thermo%theta, theta_desc)
      ! Handle horizontal-only field
      field_shape(1) = num_global_phys_cols
      call cam_grid_write_dist_array(file, grid_decomp, (/dims(1)/), (/field_shape(1)/), phys_state%slp, slp_desc)
   end subroutine restart_physics_write

   subroutine restart_physics_read()
   end subroutine restart_physics_read

end module restart_physics_ddt2
