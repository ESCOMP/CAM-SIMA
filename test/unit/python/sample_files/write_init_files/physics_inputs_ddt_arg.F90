...
!>
!! @brief Auto-generated Initial conditions source file, physics_inputs_ddt_arg.F90
!!
!
module physics_inputs_ddt_arg
...
contains
   ...
   subroutine physics_read_data(file, suite_names, timestep, read_initialized_variables)
      ...
      use physics_data,                 only: read_field, find_input_name_idx, no_exist_idx, init_mark_idx, prot_no_init_idx
      use physics_data,                 only: const_idx, read_constituent_dimensioned_field
      ...
      use phys_vars_init_check_ddt_arg, only: phys_var_num, phys_var_stdnames, input_var_names, std_name_len, is_initialized
   ...
   subroutine physics_check_data(file_name, suite_names, timestep, min_difference, min_relative_value, err_on_fail)
      ...
      use cam_abortutils,               only: endrun
      use shr_kind_mod,                 only: SHR_KIND_CS, SHR_KIND_CL, SHR_KIND_CX
      use physics_data,                 only: check_field, find_input_name_idx, no_exist_idx, init_mark_idx, prot_no_init_idx
      use physics_data,                 only: const_idx, flush_check_field_verbose
      use cam_ccpp_cap,                 only: ccpp_physics_suite_variables, cam_constituents_array, cam_model_const_properties
      ...
      use ccpp_constituent_prop_mod,    only: ccpp_constituent_prop_ptr_t
      use phys_vars_init_check_ddt_arg, only: phys_var_num, phys_var_stdnames, input_var_names, std_name_len
            ...
            select case (name_idx)
               ...
               case default
                  ...
                  case ('tendency_of_peverwhee')
                     call check_field(file, input_var_names(:,name_idx), timestep, ptend, 'tendency_of_peverwhee', &
                         min_difference, min_relative_value, is_first, diff_found)
...
end module physics_inputs_ddt_arg
