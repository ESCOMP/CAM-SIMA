...
contains
   ...
   subroutine physics_check_data(file_name, suite_names, timestep, min_difference, min_relative_value, err_on_fail)
      ...
      use ccpp_constituent_prop_mod,      only: ccpp_constituent_prop_ptr_t
      use phys_vars_init_check_ddt_array, only: phys_var_num, phys_var_stdnames, input_var_names, std_name_len
      use physics_types_ddt_array,        only: ix_theta, phys_state
         ...
         suite_required_vars: do req_idx = 1, size(ccpp_required_data, 1)
            ...
            select case (name_idx)
               ...
               case default
                  ...
                  select case (trim(phys_var_stdnames(name_idx)))
                  case ('potential_temperature')
                     call check_field(file, input_var_names(:,name_idx), 'lev', timestep, phys_state%t(:, :, ix_theta), &
